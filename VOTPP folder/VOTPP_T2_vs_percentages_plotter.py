...
for conc in concentration_list:
    ...
    for key, df in data.items():
        ...
        try:
            popt, _ =  curve_fit(coherence_time_func, time, abs(coherence), maxfev=5000, bounds=([0, -np.inf], [np.inf, np.inf]), p0=[1,0.00001])
            T2 = popt[0]
            T2_values.append(T2)
...
plt.tight_layout()
plt.savefig(f'{path}T2_vs_concentration_(E_trans).png', dpi=300)
plt.show()
