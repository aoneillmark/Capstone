...
import time
import sys
from pycce_parser import pcparser
...
calc_param = {'magnetic_field': np.array(
    [0., 0., 500.]), 'pulses': 1, 'parallel': True, 'parallel_states': True}
...
if __name__ == '__main__':
    ...
    if rank == 0:
        plt.figure()
        plt.plot(time_space, ls.T)
        plt.xlabel('Time (ms ?)')
        plt.ylabel('Coherence')
        plt.title(f'Calculation of {len(arguments.values)} {arguments.param} '
                  f'for configuration {conf}')
        plt.show()
