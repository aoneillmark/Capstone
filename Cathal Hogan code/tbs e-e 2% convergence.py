# -*- coding: utf-8 -*-
"""
Created on Sun Aug 27 09:57:22 2023
...
# Import VOTPP_opt.xyz and output True if it's imported correctly, without pandas:
with open('VOTPP_opt.xyz', 'r') as f:
    print("heehee hoohoo")
    lines = f.readlines()
    print(lines[0])
...
#parameters
nb=250 # no. of bath states
ts = np.linspace(0, 5e-3, 201) #T2 =1us for 2% concentration
...
orders.plot(title='convergence test for orders; cell=200; nbstates=250; [?:100:60]',
           xlabel='order', ylabel='Coherence')
#plt.show() #uncomment to see individually

#converge for r_bath
rbs = runner('r_bath', [80,100,120])
orders.plot(title='convergence test for r_bath; cell=200; nbstates=250; [3:?:60]',
           xlabel='order', ylabel='Coherence')
#plt.show() #uncomment to see individually

#converge for r_dipole
rds = runner('r_dipole', [40,60,80])
orders.plot(title='convergence test for r_dipole; cell=200; nbstates=250; [3:100:?]',
           xlabel='order', ylabel='Coherence')
#plt.show() #uncomment to see individually
...
plt.suptitle('Convergence tests; cell = 200copies')
fig.tight_layout()

#converge for cell size 
ls_cell = [] #empty list to collect simulations 
cell_size = [100,200,300] #enter cell sizes to be simulated 
for i in cell_size:
    #generate cell
